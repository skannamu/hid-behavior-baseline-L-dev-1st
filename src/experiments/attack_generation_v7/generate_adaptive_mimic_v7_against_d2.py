...
def parse_args():
    p = argparse.ArgumentParser(
        description="Generate AdaptiveMimic v7 candidates against D2 oracle."
    )
    ...
    p.add_argument(
        "--detector-model",
        type=Path,
        default=Path("checkpoints/recon_hid_v2_round2.pt"),
        help="D2 detector checkpoint.",
    )

    p.add_argument(
        "--candidate-dir",
        type=Path,
        default=Path("data/attack/AdaptiveMimic_v7_candidate"),
    )

    p.add_argument(
        "--result-dir",
        type=Path,
        default=Path("results/adaptive_mimic_v7_against_d2"),
    )

    p.add_argument(
        "--oracle-dir",
        type=Path,
        default=Path("reports/benchmark/adaptive_mimic_v7_candidate/oracle_d2_AdaptiveMimic_v7_candidate"),
    )
...
def mutate_row(seed_row, normal_ref_row, mutable_cols, normal_stats, rng, strategy, alpha):
    ...
    if "scenario" in cand.index:
        cand["scenario"] = "adaptive_mimic_v7"
    if "input_source" in cand.index:
        cand["input_source"] = "AdaptiveMimic_v7"
    if "session_id" in cand.index:
        cand["session_id"] = "adaptive_mimic_v7"
...
def main():
    ...
    for seed_idx, (_, seed_row) in enumerate(seeds.iterrows()):
        for _local_i in range(args.candidates_per_seed):
            ...
            if "source_file" in cand.index:
                cand["source_file"] = "v7_candidates.csv"
    ...
    candidate_csv = args.candidate_dir / "v7_candidates.csv"
    cand_df.to_csv(candidate_csv, index=False)

    meta = pd.DataFrame(metadata_rows)
    meta_path = args.result_dir / "v7_candidate_metadata.csv"
    ...
    joined = pd.concat(
        [
            meta,
            attack_preds.add_prefix("d2_"),
        ],
        axis=1,
    )

    joined_path = args.result_dir / "v7_candidate_oracle_joined.csv"
    ...
    if selected_or95:
        save_selected(selected_or95, "AdaptiveMimic_v7_D2_OR95_bypass")

    if selected_or90:
        save_selected(selected_or90, "AdaptiveMimic_v7_D2_OR90_bypass")
    ...
    if not selected_or95:
        ...
        hard_idx = tmp.sort_values("d2_attack_score", ascending=True).head(args.top_k).index.tolist()
        save_selected(hard_idx, "AdaptiveMimic_v7_D2_hardest_no_bypass")
        ...
        hard_report_path = args.result_dir / "v7_hardest_no_bypass_report.csv"
        hard_report.to_csv(hard_report_path, index=False)
        print(f"saved hard report: {hard_report_path}")

    summary = pd.DataFrame(summary_rows)
    summary_path = args.result_dir / "v7_generation_summary.csv"
